<script lang="ts">
	...
	import { AppShell, AppBar, TabGroup, TabAnchor, popup } from '@skeletonlabs/skeleton';
			...
			<AppBar background="">
				<svelte:fragment slot="lead">
					<figure class="ml-4 hidden lg:block">
						<img src="/img/delta.logo.svg" width="300" alt="Delta" />
					</figure>
					<ul class="menu lg:hidden">
						...
						<li class="list">
							<TabGroup
								...
								class=""
							>
								<TabAnchor href="./home" selected={$page.url.pathname.endsWith('/home')}>
									<span>{$t('home')}</span>
								</TabAnchor>
								<TabAnchor href="./download" selected={$page.url.pathname.endsWith('/download')}>
									<span>{$t('download')}</span>
								</TabAnchor>
								<TabAnchor href="./explore" selected={$page.url.pathname.endsWith('/explore')}>
									<span>{$t('explore')}</span>
								</TabAnchor>
								<TabAnchor
				...
				<TabGroup justify="justify-center" border="" class="hidden lg:block">
					<TabAnchor href="./home" selected={$page.url.pathname.endsWith('/home')}>
						<span>{$t('home')}</span>
					</TabAnchor>
					<TabAnchor href="./download" selected={$page.url.pathname.endsWith('/download')}>
					...
					<TabAnchor href="./whitepaper" selected={$page.url.pathname.endsWith('/whitepaper')}>
						<span>{$t('whitepaper')}</span>
					</TabAnchor>
				</TabGroup>
				<svelte:fragment slot="trail">
					<ul class="lang">
						<li class="lable">
							<i class="fa-solid fa-language text-surface-600 text-xl"></i>
							<span class="hidden md:inline">{langs[data.lang]}</span>
						</li>
						<li class="list">
							<ul>
								{#each Object.keys(langs) as key}
									<li><a href={$page.url.pathname.replace(data.lang, key)}> {langs[key]}</a></li>
								{/each}
							</ul>
						</li>
					</ul>
			...
			<div class="container mx-auto">
				<div class="flex flex-row justify-around">
					<ul>
						...
						<li><a href="./DSMSAccessTerminal">DSMS {$t('access_terminal')}</a></li>
					...
					<a class="w-1/3 md:w-80" href="./home"
						><img src="/img/delta.logo.svg" style="opacity: 0.5" alt="Delta" /></a
					>
				</div>
				<p class="text-center py-4">
					© Delta Web Project 2023-2024 Released under the MIT license
				</p>
			</div>
		</div>
	</svelte:fragment>
